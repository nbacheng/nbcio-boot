...
public class BaseCommonServiceImpl implements BaseCommonService {
    ...
    public void addLog(LogDTO logDTO) {
        ...
        try {
        	 if(logDTO.getLogContent().length()>1000) { //add by nbacheng
        		 logDTO.setLogContent(logDTO.getLogContent().substring(0,1000));
             }
            baseCommonMapper.saveLog(logDTO);
        } catch (Exception e) {
            log.warn(" LogContent length : "+logDTO.getLogContent().length());
            log.warn(" LogContent : "+logDTO.getLogContent());
    ...
    public void addLog(String logContent, Integer logType, Integer operatetype, LoginUser user) {
        ...
        if(logContent.length()>1000) { //add by nbacheng
